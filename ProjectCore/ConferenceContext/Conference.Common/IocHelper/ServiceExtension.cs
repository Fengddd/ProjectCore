...
namespace Conference.Common.IocHelper
{
    ...
    public static class ServiceExtension
    {
        ...
        /// <returns></returns>
        public static IServiceCollection RegisterDomainServiceAssembly(this IServiceCollection service, string implementAssemblyName)
        {
            if (service == null)
                throw new ArgumentNullException(nameof(service));
            if (string.IsNullOrEmpty(implementAssemblyName))
            ...
            var assembly = RuntimeHelper.GetAssembly(implementAssemblyName);
            if (assembly == null)
            {
                throw new DllNotFoundException($"the dll \"{implementAssemblyName}\" not be found");
            }
            //过滤掉非接口
            var types = assembly.GetTypes().Where(t => t.GetTypeInfo().IsClass && typeof(IDomainService).IsAssignableFrom(t.GetTypeInfo()));
